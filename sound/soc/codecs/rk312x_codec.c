...
static void rk312x_codec_unpop(struct work_struct *work)
{
	...
	rk312x_codec_ctl_gpio(CODEC_SET_HP, rk312x_priv->spk_active_level);
}
...
static int rk312x_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		...
		if (!rk312x_priv->rk312x_for_mid) {
			flush_delayed_work_sync(&rk312x_priv->spk_delayed_work);
			schedule_delayed_work(&rk312x_priv->spk_delayed_work,
...
{
	cancel_delayed_work(&rk312x_priv->linein_work);
	schedule_delayed_work(&rk312x_priv->linein_work, msecs_to_jiffies(5));
          
	return IRQ_HANDLED;
}

static void linein_work_func(struct work_struct *work)
{   
	...
	//set line in on
		...
		rockchip_line_in(RK312x_CODEC_LINE_IN);
		    ...
		    gpio_set_value(rk312x_priv->hp_ctl_gpio, rk312x_priv->spk_active_level);
	}
}
...
static int rk312x_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&rk312x_priv->hpdet_work, hpdet_work_func);
    	INIT_DELAYED_WORK(&rk312x_priv->linein_work, linein_work_func);
	INIT_DELAYED_WORK(&rk312x_priv->aux_det_work, aux_det_work_func);
	INIT_DELAYED_WORK(&rk312x_priv->spk_delayed_work, rk312x_codec_unpop);
...
static int rk312x_platform_probe(struct platform_device *pdev)
{
	...
	rk312x->spk_gpio = of_get_named_gpio_flags(rk312x_np,
						 "spk_io", 0, NULL);
	if (!gpio_is_valid(rk312x->spk_gpio)) {
		dbg_codec(2, "invalid spk_gpio: %d\n",
			  rk312x->spk_gpio);
		rk312x->spk_gpio = INVALID_GPIO;
		/* ret = -ENOENT; */
		/* goto err__; */
	}else{
		gpio_direction_output(rk312x->spk_gpio, 1);
	}
