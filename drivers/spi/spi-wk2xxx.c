...
static int wk2xxx_probe(struct spi_device *spi)
{
    ...
    ret = of_get_named_gpio_flags(node, "pwr-en-gpio", 0, NULL);
	if (ret < 0) {
		printk("%s() Can not read property pwr-en-gpio\n", __FUNCTION__);
		goto err;
	} else {
        wk2xxx_dev->pwr_en = ret;
        ret = devm_gpio_request(&spi->dev, wk2xxx_dev->pwr_en, "pwr-en-gpio");
        if(ret < 0){
			printk("%s() devm_gpio_request pwr_en request ERROR\n", __FUNCTION__);
			goto err;
		}

        ret = gpio_direction_output(wk2xxx_dev->pwr_en,1);
		if(ret < 0){
			printk("%s() gpio_direction_input pwr_en set ERROR\n", __FUNCTION__);
			goto err;
		}
    }
